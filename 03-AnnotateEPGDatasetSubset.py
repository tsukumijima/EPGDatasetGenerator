...
@app.command()
def main(
    subset_path: Annotated[Path, typer.Option(help='アノテーションを手動追加するデータセットのサブセットのパス。', dir_okay=False)] = Path('epg_dataset_subset.jsonl'),
):
    ...
    with jsonlines.open(subset_path, 'r') as reader:
        for obj in reader:
            ...
            print(f'{subset.id} のアノテーションを追加しました。')
