...
def __BuildFormatStringTranslationMap() -> None:
    """
    ...
    # すでに構築済みの場合は何もしない
    if __format_string_translation_map is not None and __format_string_regex is not None:
        return
...
def FormatString(cls, string: str) -> str:
    ...
    __BuildFormatStringTranslationMap()
    assert __format_string_translation_map is not None
    assert __format_string_regex is not None
    assert __format_string_regex_table is not None

    # 置換を実行
    result = result.translate(__format_string_translation_map)
    result = __format_string_regex.sub(lambda match: cast(dict[str, str], __format_string_regex_table)[match.group(0)], result)
